...
PolarFileName=fullfile(PolarFiles(i).folder, PolarFiles(i).name);



newStr=split(PolarFiles(i).name, '.');





PolarData= getData(PolarFileName);
...
x= reshape(x,  max(Polar(:,3)) ,max(Polar(:,1)), max(Polar(:,2)));
y= reshape(y,  max(Polar(:,3)) ,max(Polar(:,1)), max(Polar(:,2)));
z= reshape(z,  max(Polar(:,3)) ,max(Polar(:,1)), max(Polar(:,2)));

u= reshape(u,  max(Polar(:,3)) ,max(Polar(:,1)), max(Polar(:,2)));
v= reshape(v,  max(Polar(:,3)) ,max(Polar(:,1)), max(Polar(:,2)));
w= reshape(w,  max(Polar(:,3)) ,max(Polar(:,1)), max(Polar(:,2)));

saveFileName = append("Polar.", newStr{2});

%saveData(ElasticE, saveFileName)
saveVtkName = append("Polar.", newStr{2}, ".vtk");
title ='Polar';

vtkwrite(saveVtkName,'structured_grid',x,y,z,'vectors',title,u,v,w) %saves matrix m (could be 1D, 2D or 3D array) into vtk as structured points.

%vtkwrite(saveVtkName,'structured_grid',ElasticE(:,1),ElasticE(:,2),ElasticE(:,3),'scalars',title, ElasticE(:,4) )
...
% stressFiles = "PSTO_BiLayer/MartinPotential/MultiLayer/StrainLoopV3/Stress.00019000.dat"
% strainFiles = "PSTO_BiLayer/MartinPotential/MultiLayer/StrainLoopV3/Elast_St.00019000.dat"
% 
% input = importdata(stressFiles,' ',1);
% 
% 
